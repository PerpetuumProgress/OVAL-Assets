...
def run(
    ...
    line2 = pf % ("all", seen, nt.sum(), mp, mr, map50, map)
    if nt.sum() == 0:
    ...
    with open(save_dir / 'mAP_values.txt', 'w') as f:
        ...
        if (verbose or (nc < 50 and not training)) and nc > 1 and len(stats):
            for i, c in enumerate(ap_class):
                ...
                line = pf % (names[c], seen, nt[c], p[i], r[i], ap50[i], ap[i])
                f.write("%s\n" % line)
        ...
        if not training:
            ...
            line = f"Speed: %.1fms pre-process, %.1fms inference, %.1fms NMS per image at shape {shape}" % t
            f.write("%s\n" % line)
